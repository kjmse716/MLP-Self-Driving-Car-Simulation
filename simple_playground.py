...
class Car():
    ...
    def tick(self):
        ...
        print(f"updated car angle = {new_angle}")
        new_angle %= 360
        if new_angle > self.angle_max:
            new_angle -= self.angle_max - self.angle_min
...
class Playground():
    ...
    def predictAction(self, state,model,D6D):
        '''
        此function為模擬時，給予車子隨機數字讓其走動。
        不需使用此function。
        '''
        if D6D:
            cp = self.car.getPosition()              
            return model.predict([cp.x,cp.y,state[0],state[1],state[2]])
...
def run_example(model,D6D):
    ...
    while not p.done:
        ...
        action = p.predictAction(state,model,D6D)
        # take action
        state = p.step(action)
        
        pt = [p.car.getPosition('center').x,p.car.getPosition('center').y]
        poss.append(pt)
        state_log.append(state)
        action_log.append(action)
    return poss,state_log,action_log
"""         
if __name__ == "__main__":
    MLP1 = MLP.MLP(learning_rate=0.1,epoch=10)
    filepath = "E:\\Lab\\ANN\\HW2\\Documents\\作業二\\train4dAll.txt"
    MLP1.train(filepath)
    poss = run_example()
    map.plot(poss)
    #print(MLP1.predict([9.7352, 9.1366, 27.9191])) """
